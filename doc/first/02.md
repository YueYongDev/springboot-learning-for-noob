...
Spring Boot 官⽅⽀持 Maven 和 Gradle 作为项目构建⼯具。Gradle 虽然有更好的理理念，但是相⽐ Maven 来讲其行业使用率偏低，并且 Spring Boot 官⽅默认使用 Maven，因此本系列课程选择使用 Maven 作为 Spring Boot 项⽬构建⼯工具。
...
我们平时打开一个应用程序，一般是双击桌面图标或在开始菜单链接，无论是桌面的快捷图标还是菜单链接都包含了应用程序的安装位置信息，打开它们的时候系统会按照这些位置信息找到安装目录然后启动程序，这里以 Visual Studio Code 这个软件为例：

![图1.2.9 VSCode 属性](https://tva1.sinaimg.cn/large/0081Kckwgy1gkc84y57umj30n20xojts.jpg)

知道了一个应用程序的安装目录位置，我们也可以通过命令行工具打开，如笔者 VSCode 的位置为：

```
"C:\Users\lyy\AppData\Local\Programs\Microsoft VS Code\bin"
```

VSCode 的应用程序名为为 Code，那么我们打开终端工具，使用 cd 命令进入该目录，再输入`code`，即可启动 VSCode。

![图1.2.10 命令行示例](https://tva1.sinaimg.cn/large/0081Kckwly1gkc8czpof2j321q0lcn28.jpg)

然后我们只需要在**用户变量**名`Path`中添加安装路径的字符，我们在命令行工具无需进入软件的安装路径即可打开软件了。

![图1.2.11a 查看环境变量](https://tva1.sinaimg.cn/large/0081Kckwgy1gkc8kzzq5ij31cm0u07wh.jpg)

![图1.2.11b 使用环境变量](https://tva1.sinaimg.cn/large/0081Kckwgy1gkc8lrov5bj31sk0miq6p.jpg)
